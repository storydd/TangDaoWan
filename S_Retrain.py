...
train_dataset_path  =  'data/all/New 21year/21_data.npy'
train_labels_path = 'data/all/New 21year/21_label.npy'
...
X_train,X_val,y_train,y_val= train_test_split(X_train,y_train,test_size=0.25,random_state = 20,shuffle = True)
...
models[model_name]['model'].load_state_dict(torch.load('UAV_model_UAV(noAttention).pth'))
...
np.save('无人机trainloss.npy',loss_values[model_name]['train'])
np.save('无人机valloss.npy',loss_values[model_name]['val'])
# plt.title('UAV')
plt.legend()
plt.show()
# plt.savefig('result/S_loss.png')
...
# torch.save(models[model_name]['model'].state_dict(),'UAV_model(snoAttention).pth')