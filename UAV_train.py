...
def load_train_data():
    ...
    train_dataset_rw = np.concatenate((train_dataset_rw2, train_dataset_rw4), axis=0)
    ...
    train_target_rw = np.concatenate((train_target_rw2, train_target_rw4), axis=0)
...
# models[model_name]['model'].load_state_dict(torch.load('all_model_UAV.pth'))
...
torch.save(models[model_name]['model'].state_dict(),'UAV_model_UAV(noAttention).pth')