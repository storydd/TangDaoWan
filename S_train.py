...
loss_values[model_name], accuracy_values[model_name] = train(models[model_name]['path'], {'train':train_loader,'val':val_loader},
                                                             ...
                                                             scaler)
# fontprops = FontProperties(family='Times New Roman', weight='normal', size=12)
# plt.rcParams['font.family'] = 'Times New Roman'
# plt.rcParams['font.size'] = 12
plt.ylim(0, 1)
plt.plot(loss_values[model_name]['train'], label="train loss")
plt.plot(loss_values[model_name]['val'], label = "val loss")
np.save('卫星trainloss.npy',loss_values[model_name]['train'])
np.save('卫星valloss.npy',loss_values[model_name]['val'])
plt.legend()
plt.show()
...
# torch.save(models[model_name]['model'].state_dict(),'Mymodel_noAttention(S1).pth')